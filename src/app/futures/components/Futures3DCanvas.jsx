...
import LargeFluidBackground from "../../../components/LargeFluidBackground";
...
function TubeSegment({ segmentIndex, position, height, tubeRadius = 0.01, tubularSegments = 100, radialSegments = 8 }) {
  const [uniforms] = useState(() => ({
    time: { value: 0 },
  ...
  return (
    <mesh position={position}>
      ...
      <tubeGeometry args={[
        ...
        tubularSegments, // tubularSegments - 沿着曲线的段数
        tubeRadius, // radius - 管子半径
        radialSegments, // radialSegments - 径向段数
        false // closed
      ]} />
      <shaderMaterial
        attach="material"
...
function GrowthScene({ position }) {
  ...
  useFrame((state, delta) => {
    if (containerRef.current) {
      // 设置基础旋转
      containerRef.current.rotation.x = rotationX;
      containerRef.current.rotation.z = rotationZ;
      // 轻微旋转整个场景容器（在基础 rotationY 上累加）
      containerRef.current.rotation.y = rotationY + state.clock.elapsedTime * rotationSpeed;
      // 设置 Y 轴位置
      containerRef.current.position.y = positionY;
    }
  });
  ...
  const tubes = Array.from({ length: tubesCount }).map((_, i) => {
    ...
    const x = (pseudoRandom(seed) - 0.5) * positionRange;
    const z = (pseudoRandom(seed + 1) - 0.5) * positionRange;
    const height = heightBase + pseudoRandom(seed + 2) * heightRange;
...
// 单个约束管子 - 水平方向的螺旋曲线
function ConstraintTube({ tubeIndex, position, curve, tubeRadius = 0.02, tubularSegments = 100, radialSegments = 8 }) {
  const [uniforms] = useState(() => ({
    time: { value: 0 },
...
function ConstraintScene({ position }) {
  ...
  const tubes = Array.from({ length: tubesCount }).map((_, i) => {
    ...
    for (let j = 0; j <= curvePoints; j++) {
      const t = j / curvePoints; // 0 到 1
      const x = -helixLength / 2 + t * helixLength; // 从左到右
      const angle = t * Math.PI * 2 * helixTurns + angleOffset; // 螺旋圈数 + 初始偏移
...
// Futures 专用的 3D Canvas组件
export default function Futures3DCanvas({ currentSection }) {
