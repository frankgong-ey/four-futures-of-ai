...
function Globe({ counts, soundEnabled }) {
  ...
  useFrame((state, delta) => {
    if (groupRef.current) {
      ...
      groupRef.current.position.x = -1.5 + Math.sin(t * 0.6) * 0.05;
      groupRef.current.position.y = Math.sin(t * 0.5) * 0.1;
      groupRef.current.position.z = Math.sin(t * 0.4 + 1.5) * 0.05;
      const scale = 1 + Math.sin(t * 0.6 + 1) * 0.02;
